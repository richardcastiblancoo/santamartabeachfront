...
function changeLanguage(lang) {
    ...
    document.querySelectorAll('.currentLangText').forEach(el => el.innerText = lang);
    document.querySelectorAll('.currentFlag').forEach(el => {
        el.src = (lang === 'ES') ? "https://flagcdn.com/w40/co.png" : "https://flagcdn.com/w40/us.png";
    });
...
function initMapAnimation() {
    ...
    async function sequence() {
        while (true) {
            ...
            L.marker(coordsFinal).addTo(markerLayer).bindPopup(`<div class="text-center"><b>${m.popup_title}</b><br>${m.popup_desc}</div>`).openPopup();
