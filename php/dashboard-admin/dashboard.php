...
<body class="bg-background-light dark:bg-background-dark text-text-main dark:text-white font-display overflow-hidden">
    ...
    <?php
    // 1. DIAGNÓSTICO DE ERRORES (Si sale un error, PHP te dirá cuál es)
    ...
    // --- CONSULTAS PARA LAS TARJETAS (MODO SEGURO) ---
    $total_reservas = ($res = $conn->query("SELECT COUNT(*) as count FROM reservas")) ? $res->fetch_assoc()['count'] : 0;
    $total_pqr = ($res = $conn->query("SELECT COUNT(*) as count FROM pqr")) ? $res->fetch_assoc()['count'] : 0;
    $pqr_pendientes = ($res = $conn->query("SELECT COUNT(*) as count FROM pqr WHERE estado = 'Pendiente'")) ? $res->fetch_assoc()['count'] : 0;
    $total_usuarios = ($res = $conn->query("SELECT COUNT(*) as count FROM usuarios WHERE rol != 'Admin'")) ? $res->fetch_assoc()['count'] : 0;

    // Obtener apartamentos (Límite 6 para el dashboard)
    $apartamentos_res = $conn->query("SELECT * FROM apartamentos LIMIT 6");

    // Obtener reservas recientes
    $reservas_res = $conn->query("SELECT r.*, u.nombre, u.apellido, u.email, a.titulo, a.imagen_principal 
                             FROM reservas r 
        ...
        <script id="tailwind-config">
            tailwind.config = {
                ...
                theme: {
                    extend: {
                        colors: {
                            "primary": "<?php echo $_SESSION['tema'] ?? '#13a4ec'; ?>",
                            "primary-hover": "<?php echo $_SESSION['tema'] ?? '#0e8ac7'; ?>",
                            "background-light": "#f6f7f8",
                            "background-dark": "#101c22",
                            "card-light": "#ffffff",
                            "card-dark": "#1a2c35",
            ...
            <div class="flex flex-col flex-1 min-w-0">
                ...
                <main class="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 scroll-smooth">
                    <section id="dashboard-section">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                            <div class="bg-card-light dark:bg-card-dark p-5 rounded-xl border border-[#f0f3f4] dark:border-gray-800 shadow-sm flex flex-col justify-between card-hover-effect">
                                <div class="size-10 bg-primary/10 rounded-lg flex items-center justify-center mb-4">
                                    <span class="material-symbols-outlined text-primary">calendar_month</span>
                                </div>
                                <div>
                                    <h3 class="text-text-secondary text-sm font-medium">Total de Reservas</h3>
                                    <span class="text-3xl font-bold"><?php echo $total_reservas; ?></span>
                                </div>
                            </div>
                            <div class="bg-card-light dark:bg-card-dark p-5 rounded-xl border border-[#f0f3f4] dark:border-gray-800 shadow-sm flex flex-col justify-between card-hover-effect">
                                <div class="size-10 bg-orange-100 dark:bg-orange-900/30 rounded-lg flex items-center justify-center mb-4">
                                    <span class="material-symbols-outlined text-orange-600">contact_support</span>
                                </div>
                                <div>
                                    <h3 class="text-text-secondary text-sm font-medium">Total de PQR</h3>
                                    <span class="text-3xl font-bold"><?php echo $total_pqr; ?></span>
                                </div>
                            </div>
                        </div>
                    </section>

                    <section id="apartments-section" class="pt-8 border-t border-[#f0f3f4] dark:border-gray-800">
                        <h2 class="text-xl font-bold mb-6 flex items-center gap-3">
                            <span class="material-symbols-outlined text-primary">apartment</span> Gestión de Apartamentos
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            <?php if ($apartamentos_res): ?>
                                <?php while ($apt = $apartamentos_res->fetch_assoc()): ?>
                                    <div class="bg-card-light dark:bg-card-dark p-4 rounded-xl border border-[#f0f3f4] dark:border-gray-800 shadow-sm flex items-center gap-4 hover:border-primary/30 transition-colors">
                                        <div class="w-20 h-20 rounded-lg bg-cover bg-center shrink-0" style='background-image: url("<?php echo !empty($apt['imagen_principal']) ? '../../assets/img/apartamentos/' . $apt['imagen_principal'] : 'https://placehold.co/400'; ?>");'></div>
                                        <div class="flex-1 min-w-0">
                                            <h4 class="font-bold truncate"><?php echo $apt['titulo']; ?></h4>
                                            <p class="text-[11px] text-text-secondary"><?php echo $apt['ubicacion']; ?></p>
                                            <div class="mt-2 flex gap-3">
                                                <button onclick="openViewModal(<?php echo $apt['id']; ?>)" class="text-[11px] font-medium text-text-secondary hover:text-primary flex items-center gap-1">
                                                    <span class="material-symbols-outlined text-xs">visibility</span> Ver
                                                </button>
                                                <button onclick="openEditModal(<?php echo $apt['id']; ?>)" class="text-[11px] font-medium text-text-secondary hover:text-primary flex items-center gap-1">
                    ...
                    <section id="pqr-section" class="pt-8 border-t border-[#f0f3f4] dark:border-gray-800 pb-16">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="text-xl font-bold flex items-center gap-3">
                                <span class="material-symbols-outlined text-primary">mail</span> Bandeja de PQR
                            </h2>
                            <?php if ($pqr_pendientes > 0): ?>
                                <span class="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full"><?php echo $pqr_pendientes; ?> Nuevas</span>
                            <?php endif; ?>
                        </div>
                        <div class="bg-card-light dark:bg-card-dark rounded-xl border border-[#f0f3f4] dark:border-gray-800 divide-y divide-[#f0f3f4] dark:divide-gray-800">
                            <?php if ($pqr_res): ?>
                                <?php while ($pqr = $pqr_res->fetch_assoc()): ?>
                                    <div class="p-4 hover:bg-background-light dark:hover:bg-gray-800 transition-colors flex items-start gap-4">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex justify-between items-baseline mb-0.5">
                                                <p class="font-bold text-sm"><?php echo $pqr['nombre'] . ' ' . $pqr['apellido']; ?></p>
                                                <p class="text-[10px] text-text-secondary uppercase"><?php echo date('d M H:i', strtotime($pqr['fecha_creacion'])); ?></p>
                                            </div>
                                            <h5 class="text-xs font-semibold mb-0.5"><?php echo $pqr['asunto']; ?></h5>
                                            <p class="text-xs text-text-secondary line-clamp-1"><?php echo $pqr['mensaje']; ?></p>
                                            <button onclick="openResponseModal(<?php echo $pqr['id']; ?>, '<?php echo addslashes($pqr['asunto']); ?>', '<?php echo addslashes($pqr['nombre'] . ' ' . $pqr['apellido']); ?>')" class="mt-2 text-[10px] font-bold bg-primary text-white px-3 py-1 rounded">Responder</button>
                                        </div>
                                    </div>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        ...
                        <div class="px-4 pt-5 pb-4 bg-white dark:bg-card-dark sm:p-6 sm:pb-4">
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="titulo" id="edit_titulo" class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white">
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Ubicación</label>
                                        <input type="text" name="ubicacion" id="edit_ubicacion" class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Precio</label>
                                        <input type="number" name="precio" id="edit_precio" class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Descripción</label>
                                    <textarea name="descripcion" id="edit_descripcion" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit" class="w-full inline-flex justify-center rounded-md bg-primary text-white px-4 py-2 text-sm font-medium">Guardar Cambios</button>
                            <button type="button" onclick="closeEditModal()" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium dark:text-white">Cancelar</button>
                        </div>
                    </form>
                </div>
            </div>
